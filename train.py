...
class VideoClassifier():
    ...
    def __init__(self, config_path):
        ...
        self.logger.info(f"Train log for model with backbone {config['backbone']}")
                ...
                for fpath in data_paths[offset:offset + batch_size]:
                    ...
                    preprocessed_frames = np.asarray(
                        [self.preprocess_func(cv2.resize(frame, (img_size, img_size)))
                         for frame in copy.deepcopy(video_feats)])
                    frames_batch.append(preprocessed_frames)
                    labels_batch.append(self.config["CLASS_NAME_TO_LABEL"][fpath.split('/')[-2]])
                yield (np.asarray(frames_batch), np.asarray(masks_batch)), to_categorical(labels_batch, num_classes=self.n_classes)
...
def main():
    ...
    model = VideoClassifier(args.config_path)
    model.build_model()
    model.train()
